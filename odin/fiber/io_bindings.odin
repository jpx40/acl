...
foreign libfiber {
	...
	acl_fiber_read :: proc(sock: socket_t, buf: [^]u8, count: c.size_t) -> c.ssize_t ---
	...
	acl_fiber_recvfrom :: proc(sock: socket_t, buf: [^]u8, len: c.size_t, flags: c.int, src_addr: ^posix.sockaddr, addrlen: ^posix.socklen_t) -> c.ssize_t ---
    ...
    acl_fiber_poll :: proc(pollfd :[^]os.pollfd,  nfds: c.uint,  timeout: c.int) -> c.int---
	...
	acl_fiber_pread :: proc(fd: socket, buf: [^]u8, count: c.size_t, offset: c.longlong) -> c.ssize_t ---

	__acl_fiber_write :: proc(fd: socket, buf: [^]u8, count: c.size_t, offset: c.longlong) -> c.ssize_t ---
	acl_fiber_unlink :: proc(path: cstring) -> c.int ---
